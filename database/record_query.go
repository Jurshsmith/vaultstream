...
func (rq *RecordQuery) QuerySignature() *SignatureQuery {
	query := (&SignatureClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, false, record.SignatureTable, record.SignatureColumn),
		)
...
func (rq *RecordQuery) Clone() *RecordQuery {
	...
	return &RecordQuery{
		...
		withSignature: rq.withSignature.Clone(),
...
func (rq *RecordQuery) WithSignature(opts ...func(*SignatureQuery)) *RecordQuery {
	query := (&SignatureClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withSignature = query
	return rq
}
...
//
//	client.Record.Query().
//		GroupBy(record.FieldInsertedAt).
...
func (rq *RecordQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Record, error) {
	...
	if query := rq.withSignature; query != nil {
		if err := rq.loadSignature(ctx, query, nodes, nil,
			func(n *Record, e *Signature) { n.Edges.Signature = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RecordQuery) loadSignature(ctx context.Context, query *SignatureQuery, nodes []*Record, init func(*Record), assign func(*Record, *Signature)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(signature.FieldRecordID)
	}
	query.Where(predicate.Signature(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(record.SignatureColumn), fks...))
	}))
