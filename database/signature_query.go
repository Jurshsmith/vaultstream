...
func (sq *SignatureQuery) QueryRecord() *RecordQuery {
...
func (sq *SignatureQuery) WithRecord(opts ...func(*RecordQuery)) *SignatureQuery {
	query := (&RecordClient{config: sq.config}).Query()
...
//		GroupBy(signature.FieldRecordID).
...
//
//	client.Signature.Query().
//		Select(signature.FieldRecordID).
//		Scan(ctx, &v)
...
func (sq *SignatureQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Signature, error) {
	...
	if query := sq.withRecord; query != nil {
		if err := sq.loadRecord(ctx, query, nodes, nil,
			func(n *Signature, e *Record) { n.Edges.Record = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SignatureQuery) loadRecord(ctx context.Context, query *RecordQuery, nodes []*Signature, init func(*Signature), assign func(*Signature, *Record)) error {
...
func (sq *SignatureQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := sq.ctx.Fields; len(fields) > 0 {
		...
		if sq.withRecord != nil {
			_spec.Node.AddColumnOnce(signature.FieldRecordID)
		}
	}
