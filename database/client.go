...
func (c *Client) Intercept(interceptors ...Interceptor) {
	...
	c.Signature.Intercept(interceptors...)
}
...
// QuerySignature queries the signature edge of a Record.
func (c *RecordClient) QuerySignature(r *Record) *SignatureQuery {
...
func (c *RecordClient) mutate(ctx context.Context, m *RecordMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("database: unknown Record mutation op: %q", m.Op())
	}
}
...
// QueryRecord queries the record edge of a Signature.
func (c *SignatureClient) QueryRecord(s *Signature) *RecordQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, true, signature.RecordTable, signature.RecordColumn),
		)
...
func (c *SignatureClient) mutate(ctx context.Context, m *SignatureMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("database: unknown Signature mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		Record, Signature []ent.Hook
	}
	inters struct {
		Record, Signature []ent.Interceptor
	}
)
