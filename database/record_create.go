...
func (rc *RecordCreate) SetInsertedAt(t time.Time) *RecordCreate {
	rc.mutation.SetInsertedAt(t)
	return rc
}

// SetNillableInsertedAt sets the "inserted_at" field if the given value is not nil.
func (rc *RecordCreate) SetNillableInsertedAt(t *time.Time) *RecordCreate {
	if t != nil {
		rc.SetInsertedAt(*t)
	}
	return rc
...
func (rc *RecordCreate) SetSignatureID(id int) *RecordCreate {
	rc.mutation.SetSignatureID(id)
	return rc
}

// SetNillableSignatureID sets the "signature" edge to the Signature entity by ID if the given value is not nil.
func (rc *RecordCreate) SetNillableSignatureID(id *int) *RecordCreate {
	if id != nil {
		rc = rc.SetSignatureID(*id)
	}
	return rc
}

// SetSignature sets the "signature" edge to the Signature entity.
func (rc *RecordCreate) SetSignature(s *Signature) *RecordCreate {
	return rc.SetSignatureID(s.ID)
}
...
func (rc *RecordCreate) defaults() {
	if _, ok := rc.mutation.InsertedAt(); !ok {
		v := record.DefaultInsertedAt()
		rc.mutation.SetInsertedAt(v)
	}
}
...
func (rc *RecordCreate) check() error {
	if _, ok := rc.mutation.InsertedAt(); !ok {
		return &ValidationError{Name: "inserted_at", err: errors.New(`database: missing required field "Record.inserted_at"`)}
	}
	return nil
}
...
func (rc *RecordCreate) createSpec() (*Record, *sqlgraph.CreateSpec) {
	...
	if value, ok := rc.mutation.InsertedAt(); ok {
		_spec.SetField(record.FieldInsertedAt, field.TypeTime, value)
		_node.InsertedAt = value
	}
	if nodes := rc.mutation.SignatureIDs(); len(nodes) > 0 {
